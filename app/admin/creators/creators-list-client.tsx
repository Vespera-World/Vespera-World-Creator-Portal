...
interface CreatorsListClientProps {
  creators: (Client & { pending_tasks: number; social_links: ClientSocialLink[]; total_followers: number })[]
  isDemo?: boolean
}
...
export function CreatorsListClient({ 
  ...
  const filteredCreators = creators.filter(creator => {
    ...
    // Use crm_status for filtering (fallback to status for legacy data)
    const creatorStatus = creator.crm_status || creator.status
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="pill-gold">
            ...
            {creators.filter(c => c.crm_status === 'active' || (!c.crm_status && c.status === 'active')).length} Active
          </div>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`pill text-[10px] ${(creator.crm_status || creator.status) === 'active' ? 'pill-success' : (creator.crm_status || creator.status) === 'prospect' ? 'pill-warning' : 'pill-danger'}`}>
                      {creator.crm_status || creator.status}
                    </span>
                  </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold">{(creator.total_followers || creator.subscribers || 0).toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {creator.social_links?.length > 0 
                      ? `${creator.social_links.length} platform${creator.social_links.length > 1 ? 's' : ''}`
                      : creator.platform || 'Multi-platform'
                    }
                  </p>
                </div>
              </div>
