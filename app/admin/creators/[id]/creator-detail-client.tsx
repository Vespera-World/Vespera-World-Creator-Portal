...
export function CreatorDetailClient({ 
  ...
  const getPlatformIcon = (platform: string) => {
    ...
    if (p.includes('tiktok')) return <Globe className="h-4 w-4" />
    if (p.includes('onlyfans') || p.includes('fansly')) return <Users className="h-4 w-4" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <span className={`pill ${(creator.crm_status || creator.status) === 'active' ? 'pill-success' : (creator.crm_status || creator.status) === 'prospect' ? 'pill-warning' : 'pill-danger'}`}>
        ...
        <div className="glass-card p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{(totalFollowers || creator.subscribers || 0).toLocaleString()}</p>
            </div>
          ...
          <p className="text-xs text-muted-foreground mt-3">
            {socialLinks.length > 0 
              ? `Across ${socialLinks.length} platform${socialLinks.length > 1 ? 's' : ''}`
              : `On ${creator.platform || 'all platforms'}`
            }
          </p>
        </div>
            ...
            {socialLinks.map((link) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{link.platform}</p>
