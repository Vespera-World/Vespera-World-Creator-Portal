...
export function AdminContentClient({ 
  ...
  // Filter by creator
  const filterByCreator = <T extends { creator_id?: string | null; client_id?: string | null }>(items: T[]): T[] => {
    if (selectedCreator === "all") return items
    return items.filter(item => (item.creator_id || item.client_id) === selectedCreator)
  }
